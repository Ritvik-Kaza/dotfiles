...
```
- `fzf` / `fd` / `bat`: fuzzy file finder used from Bash (Ctrl+T / Alt+C) and by the Super+Shift+R floating popup below.
...
- `chromium` (optional): only needed for `webapp-install`, which turns any website into a chromeless launcher app. The script hardcodes `DRI_PRIME=0 LIBVA_DRIVER_NAME=iHD` plus a set of GPU flags (`--use-angle=gl --disable-features=Vulkan,VulkanFromANGLE`) that work around a Zink/Vulkan driver incompatibility specific to this machine's Intel + Nvidia (Nouveau) hybrid graphics — these flags may be unnecessary or wrong on different hardware; if webapp launches are slow or erroring elsewhere, try removing them first. `intel-media-driver` is also needed for hardware video decode (VAAPI) to work correctly in these webapp windows.
...
- **`webapp-install <Name> <url>`** turns any website into a chromeless launcher app: creates a `.desktop` entry running `chromium --app=<url> --class=<Name>`, fetches a real favicon via Google's favicon service and installs it into `~/.local/share/icons/hicolor/128x128/apps/` (falls back to a generic icon if the fetch fails), and gives the window a predictable class for Hyprland window rules. The generated `.desktop` file and icon are per-machine artifacts, not tracked in this repo — re-run the script on any new machine. The exec line is prefixed with `DRI_PRIME=0 LIBVA_DRIVER_NAME=iHD` and includes `--use-angle=gl --disable-features=Vulkan,VulkanFromANGLE`, all specific workarounds for this machine's Intel + Nvidia (Nouveau) hybrid graphics, where Nouveau's incomplete Vulkan support on this Ampere-generation GPU causes a `ZINK: vkCreateInstance failed (VK_ERROR_INCOMPATIBLE_DRIVER)` error and several extra seconds of startup delay on every launch; these flags cut launch time roughly in half (~7.4s to ~4.3s) but don't eliminate the underlying error, which would require switching off Nouveau to Nvidia's proprietary driver — deliberately not done, since this machine's GPU-heavy workloads (gaming, compute) are handled on a separate Windows dual-boot install instead.
