...
├── theme-presets/
│   ├── dark/              # Default theme: 7 files — waybar/wlogout/rofi/hyprlock/alacritty/hyprpaper/theme-switcher styles
│   ├── rose/              # Dusty-rose accent theme, same 7 files, wallpaper3
│   └── nokron/            # Violet/indigo with warm gold accent, same 7 files, wallpaper4
├── theme-switcher/
│   ├── layout             # wlogout layout for the theme picker (Dark / Rose / Nokron buttons)
...
- **`theme-presets/dark/`** — wallpaper is `wallpaper2.png`; **`theme-presets/rose/`** — wallpaper is `wallpaper3.jpeg`; **`theme-presets/nokron/`** — wallpaper is `wallpaper4.png`. Both `hyprlock.conf` and `hyprpaper.conf` in each preset must agree on the path, since `theme-switch` reads the wallpaper path *from* the preset's `hyprlock.conf` to feed to `hyprctl hyprpaper`.
...
- Theme switching: a paintbrush icon sits leftmost in waybar's right-side module group. Clicking it runs `theme-menu`, which opens a small wlogout-based popup (same mechanism as `power-menu`) listing "Dark", "Rose", and "Nokron" (violet/indigo with a warm gold accent, inspired by a starry-ruins wallpaper). Picking one runs `theme-switch <name>`, which copies that preset's 7 files (waybar, wlogout, rofi, hyprlock, alacritty, hyprpaper, and the theme-switcher popup's own style) into place, restarts waybar (`SIGUSR2`) and hyprpaper, and fires a notification. Each preset's `action` in `theme-switcher/layout` wraps the command in `setsid sh -c '...'` — without `setsid`, wlogout kills the spawned process when its own window closes (which happens immediately after a click), so anything past the first couple of fast commands (like the `hyprctl hyprpaper` calls) would silently never run. Alacritty only picks up new colors on freshly-opened windows; already-open terminals need to be closed and reopened. The picker popup's own background/border themes along with everything else, but each button's *hover color* (blue for Dark, rose for Rose, violet for Nokron) stays fixed across all three presets by design — it identifies which theme that button switches *to*, not the currently active one.
