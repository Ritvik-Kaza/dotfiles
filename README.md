...
- **`theme-presets/dark/`** — wallpaper is `wallpaper2.png`; **`theme-presets/rose/`** — wallpaper is `wallpaper3.jpeg`; **`theme-presets/nokron/`** — wallpaper is `wallpaper4.png`; **`theme-presets/krat/`** — wallpaper is `wallpaper5.png`. Both `hyprlock.conf` and `hyprpaper.conf` in each preset must agree on the path, since `theme-switch` reads the wallpaper path *from* the preset's `hyprlock.conf` to feed to `hyprctl hyprpaper`.
